...
async function handleRequest(request, response) {
  ...
  if (request.method === 'POST' && pathname === '/api/auth/verify-student') {
    const body = await readJsonBody(request);
    if (!hasText(body.prn) || !isValidDate(body.dateOfBirth)) {
      return sendError(response, 400, 'A PRN and valid date of birth are required.');
    }
    return sendJson(response, 200, { verified: true, mode: 'local-development' });
  }
